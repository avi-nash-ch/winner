<?php
class AppMasterCategory_model extends MY_Model
{

    public function All()
    {
        $this->db->select('app_tbl_category.*');
        $this->db->from('app_tbl_category');
    ...
    {
        $this->db->insert('app_tbl_category', $data);
    ...
    public function ViewTableMaster($id)
    {
        $Query = $this->db->where('isDeleted', False)
            ->where('id', $id)
            ->get('app_tbl_category');
        return $Query->row();
    }
    public function UpdateTableMaster($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update('app_tbl_category', $data);
