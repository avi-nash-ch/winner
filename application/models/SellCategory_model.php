<?php
class SellCategory_model extends MY_Model
{

    public function All()
    {
        $this->db->select('tbl_sell_category.*');
        $this->db->from('tbl_sell_category');
    ...
    public function ViewTableMaster($id)
    {
        $Query = $this->db->where('isDeleted', False)
            ->where('id', $id)
            ->get('tbl_sell_category');
        return $Query->row();
    }
    
    public function AddTableMaster($data)
    {
        $this->db->insert('tbl_sell_category', $data);
    ...
    public function Delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tbl_sell_category', $data);
