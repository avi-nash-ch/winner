<?php
class AppMasterSubCategory_model extends MY_Model
{

    public function All()
    {
        $this->db->select('app_tbl_subcategory.*');
        $this->db->from('app_tbl_subcategory');
    ...
    {
        $this->db->insert('app_tbl_subcategory', $data);
    ...
    public function ViewTableMaster($id)
    {
        $Query = $this->db->where('isDeleted', False)
            ->where('id', $id)
            ->get('app_tbl_subcategory');
        return $Query->row();
    }
    public function UpdateTableMaster($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update('app_tbl_subcategory', $data);
